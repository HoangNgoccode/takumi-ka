    ...
    <section id="menu" className="bg-[hsl(var(--parchment))] py-16 md:py-24">
      <div className="max-w-6xl mx-auto px-6 md:px-10">
        <div className="text-center mb-10 md:mb-14">
          <p className="text-[11px] uppercase tracking-[0.4em] text-[hsl(var(--wood))] mb-3">La Crème · Tủ bánh</p>
          <h2 className="font-heading text-3xl md:text-4xl text-balance">Bánh cuộn kem</h2>
          <p className="mt-4 text-sm text-foreground/60">Từng cuộn bánh được nghệ nhân làm tỉ mỉ. Hãy chọn hương vị bạn yêu thích.</p>
        </div>

        {creamRolls.length === 0 ? (
          <p className="text-center text-sm text-foreground/50 py-12">Đang chuẩn bị, xin vui lòng chờ.</p>
        ) : (
            ...
            {creamRolls.map((p) => (
              <li key={p.id} className="group">
                ...
                <button onClick={() => addItem(p)} className="mt-4 w-full inline-flex items-center justify-center gap-2 min-h-[44px] rounded-full border hairline text-sm hover:bg-[hsl(var(--wood))] hover:text-[hsl(var(--parchment))] hover:border-[hsl(var(--wood))] transition-colors">
                  <Plus className="w-4 h-4" strokeWidth={1.5} /> Thêm vào đơn
                </button>
              </li>
            ))}
          </motion.ul>
