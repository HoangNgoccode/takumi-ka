    ...
    <>
      ...
      <aside className={`fixed top-0 right-0 z-50 h-full w-full max-w-md bg-[hsl(var(--parchment))] shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`} role="dialog" aria-label="注文内容">
        <div className="flex items-center justify-between px-6 md:px-8 h-16 md:h-20 border-b hairline">
          <div><p className="text-[11px] uppercase tracking-[0.3em] text-muted-foreground">Your Order</p><h2 className="font-heading text-lg">お選びいただいたもの</h2></div>
        ...
        <div className="border-t hairline px-6 md:px-8 py-6 space-y-4">
          ...
          <button disabled={items.length === 0} className="w-full min-h-[52px] rounded-full bg-[hsl(var(--wood))] text-[hsl(var(--parchment))] font-heading hover:bg-[hsl(var(--espresso))] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">選択を確認</button>
          {items.length > 0 && <button onClick={clear} className="w-full text-xs text-muted-foreground hover:text-[hsl(var(--wood))] underline underline-offset-4">すべて削除</button>}
        </div>
      </aside>
    </>
  );
